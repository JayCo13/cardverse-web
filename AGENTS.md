...
- **A seller's only shipping decisions are which couriers, and how they pack** (`ShopShippingSetup` on `/sell` and the listing wizard's step 2 for carriers; `PUT /api/shipping/shop-shipping` takes `carriers` and/or `parcelOverrides`). The booking desk shows the kind dropdown beside editable weight/dimensions prefilled from the kind, with "save as my default for this type" writing `parcel_overrides[kind]`; checkout then quotes buyers with the seller's numbers. `profiles.parcel_preset` and `cards.parcel_preset` are unused since the same day. Carriers that do not collect at the pickup address are greyed from `profiles.carrier_coverage`, probed against five destinations by `refreshCarrierCoverage` when the pickup is saved or after seven days (`POST /api/shipping/coverage` re-probes). An empty tick list means shipping is not configured; `shipmentCarriers` must never silently expand it to every collecting courier. **Saving requires at least `MIN_SHOP_CARRIERS` (2) ticked** (since 2026-09-13), so the seller always has a backup courier when the buyer's stops serving the route; `minShopCarriers(coverage)` lowers the floor only when fewer than two collect at the pickup address, and the setup form, `PUT /api/shipping/shop-shipping` and the wizard's readiness check all read that one helper. Offerable couriers: GHN, SPX, J&T, BEST (`offerable: true` in `shipping-carriers.ts`); VNPost and hand delivery stay defined for old orders only.
