/**
 * PSA Pokemon English Cards Crawler (Set-Based)
 * 
 * Crawls eBay for PSA graded Pokemon English cards by set.
 * Gets top 10 most expensive cards per set and searches for PSA 10, 9, 8.
 * Uses card number for precise matching (e.g., "Charizard ex 110/025").
 * 
 * Usage: npx tsx scripts/crawl-psa-pokemon-en.ts [--sets=10] [--cards-per-set=10] [--max-api-calls=500]
 * 
...
// Category ID for English Pokemon
const CATEGORY_ID = 3;
...
async function fetchSets(): Promise<PokemonSet[]> {
    console.log(`📚 Fetching Pokemon English sets (limit ${MAX_SETS})...`);

    // Get distinct sets from tcgcsv_groups for Pokemon category
...
function buildSearchQuery(card: TargetCard, grade: string): string {
    ...
    // Include card number if available (format: 284 217 instead of 284/217)
    const cardNum = card.number ? card.number.replace('/', ' ') : '';

    // Simple query: PSA + grade + name + number + pokemon
    // No quotes - let eBay do fuzzy matching
    return `PSA ${grade} ${nameParts} ${cardNum} pokemon`.trim();
}

async function main() {
    console.log('🚀 PSA Pokemon English Cards Crawler (Set-Based)');
    console.log('=================================================');
