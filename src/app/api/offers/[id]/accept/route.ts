...
// from the client. Returns { transactionId, conversationId } for the caller to
// route into the transaction room.
export async function POST(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
    ...
    const { data: lockedCard, error: lockError } = await supabase
        .from('cards')
        .update({ status: 'in_transaction' } as never)
        .eq('id', cardRow.id)
        .eq('status', 'active')
        .select('id')
        .maybeSingle();
    ...
    if (txError || !transaction) {
        // Roll the card back so it does not get stuck if the insert failed.
        await supabase.from('cards').update({ status: 'active' } as never).eq('id', cardRow.id);
        return NextResponse.json({ error: txError?.message || 'Không thể tạo giao dịch.' }, { status: 400 });
    }
    ...
    await supabase
        .from('offers')
        .update({ status: 'chosen', transaction_id: transactionId } as never)
        .eq('id', offerRow.id);
    ...
    if (conversationId) {
        const systemBody = `Người bán đã chấp nhận đề nghị ${formatVND(offerRow.price)}. Vui lòng hoàn tất thanh toán trực tiếp trên CardVerse.`;
        ...
        const { data: systemMessage } = await supabase
            ...
            .insert({
                ...
                body: systemBody,
                message_type: 'system',
                metadata: { offerId: offerRow.id, cardId: cardRow.id, transactionId },
            } as never)
    ...
    await supabase.from('notifications').insert({
        ...
        message: `Người bán đã chấp nhận đề xuất ${formatVND(offerRow.price)} của bạn. Vào phòng giao dịch ngay!`,
        card_id: cardRow.id,
        offer_id: offerRow.id,
        conversation_id: conversationId,
    ...
    return NextResponse.json({ transactionId, conversationId });
}
