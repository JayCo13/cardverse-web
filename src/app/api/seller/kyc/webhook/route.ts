...
import { getKycProvider, hashDocumentNumber, TERMINAL_KYC_STATUSES } from '@/lib/kyc';
...
/** Statuses that carry a decision worth fetching in full. */
const DECISION_BEARING: readonly KycStatus[] = ['Approved', 'Declined', 'In Review', 'Abandoned'];
...
export async function POST(request: NextRequest) {
    ...
    if (DECISION_BEARING.includes(event.status)) {
        try {
            ...
            update.verified_full_name = identity.fullName;
            update.verified_dob = identity.dateOfBirth;
            update.verified_document_type = identity.documentType;
            update.verified_issuing_state = identity.issuingState;
            update.document_number_hash = hashDocumentNumber(identity.documentNumber);
            update.liveness_score = identity.livenessScore;
            update.face_match_score = identity.faceMatchScore;
            update.nfc_verified = identity.nfcVerified;
            update.warnings = identity.warnings;
            update.decision = decision.raw;
        } catch (error) {
