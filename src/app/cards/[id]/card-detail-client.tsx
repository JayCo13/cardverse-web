...
/**
 * The interactive listing page. `initialCard` is what the server component
 * already fetched for the first HTML: with it the page renders the listing
 * straight away and the crawler sees the name, price and images; the client
 * still re-fetches on mount to apply the viewer-specific checks (hidden
 * listings, the owner's view) and to pick up anything that changed since.
 */
export default function CardDetailClient({ initialCard = null }: { initialCard?: Card | null }) {
    ...
    const [isLoading, setIsLoading] = useState(initialCard === null);
    const [offers, setOffers] = useState<Offer[]>([]);
    const [activeImage, setActiveImage] = useState(initialCard?.imageUrl || initialCard?.imageUrls?.[0] || "");
