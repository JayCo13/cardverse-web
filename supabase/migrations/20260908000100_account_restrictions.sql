-- Account restrictions deliberately do not use GoTrue banned_until or user metadata.
-- Apply to staging and verify before enabling ACCOUNT_RESTRICTIONS_ENABLED in admin.
begin;
...
create table public.account_restriction_events (
...
create table public.account_review_holds (
  id uuid primary key default gen_random_uuid(),
...
create index on public.account_review_holds(order_id) where resolved_at is null;
create index on public.account_review_holds(withdrawal_id) where resolved_at is null;
...
$$;
-- All multi-user locks use UUID order. The transaction lock also coordinates
-- financial RPCs which acquire wallet/order/card locks in different legacy orders.
-- This conservative v1 serialization can be narrowed after concurrency testing.
create function public.lock_account_operations(p_users uuid[]) returns void
...
$$;
create function public.assert_active_account(p_user_id uuid) returns void
language plpgsql security definer set search_path = public, pg_temp as $$
begin
...
$$;
create function public.assert_account_session() returns void
language plpgsql security definer set search_path = public, pg_temp as $$
begin
...
$$;
create function public.set_account_restriction(p_user_id uuid, p_action text, p_reason text,
...
create function public.immutable_account_audit() returns trigger language plpgsql as $$
...
-- Protect direct PostgREST writes, even when an existing policy permits them.
-- Protect definer writes too: RLS alone does not apply to SECURITY DEFINER.
create function public.guard_account_write() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
-- Hide purchasable inventory without mutating status or historical snapshots.
...
-- Statement-level lock runs before row locks, including service-role webhook work.
create function public.lock_account_statement() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
...
create function public.guard_restricted_trade() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
...
begin
  ...
  if TG_OP='INSERT' then
    -- New checkout orders must not cross a ban. Late payments update staged orders.
...
-- Payment retries are allowed and acquire the same lock as a ban. Holds are
-- attached in the payment transaction before notifications/fulfilment can run.
create function public.hold_restricted_payment() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
begin
  insert into public.account_review_holds(event_id,user_id,order_id)
...
create function public.guard_restricted_withdrawal() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
-- Inject the session boundary and transaction lock at entry, before legacy row
-- locks. Keep existing financial/evidence/idempotency logic intact. SQL definers
-- are protected by the PostgREST pre-request boundary below as well as RLS/writes.
do $$
...
begin
  for f in select p.oid,p.proname from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    ...
    where n.nspname='public' and p.prosecdef and l.lanname='plpgsql'
      ...
      and p.proname not in ('account_is_active','assert_active_account','assert_account_session','lock_account_operations','set_account_restriction') loop
...
create function public.account_request_guard() returns void language plpgsql security definer set search_path=public,pg_temp as $$
begin
...
$$;
-- Do not silently replace another project's pre-request authentication hook.
do $$
begin
...
-- Keep internal audit and helpers unavailable for arbitrary client RPC calls.
...
-- account_is_active exposes a boolean only, never the reason or history.
...
-- Reuse the existing reviewed settlement function, extending only its eligibility
-- check. A separate audited wrapper resolves holds in the same transaction.
do $$
...
create function public.resolve_account_order_hold(p_order_id uuid,p_action text,p_reason text,p_actor_id text,p_actor_role text,p_key uuid)
...
create function public.resolve_account_withdrawal_hold(p_withdrawal_id uuid,p_reason text,p_actor_id text,p_actor_role text,p_key uuid)
returns jsonb language plpgsql security definer set search_path=public,pg_temp as $$
...
-- Guard service-role spending/payment-link creation too. Webhook credits/refunds
-- deliberately do not use this guard, and retain existing verified-funds checks.
do $$
...
begin
  for f in select p.oid,p.proname,p.proargnames from pg_proc p join pg_namespace n on n.oid=p.pronamespace
    where n.nspname='public' and p.proname in ('spend_verified_wallet','create_server_payment_order','stage_payos_marketplace_checkout','create_verified_wallet_marketplace_orders','claim_payos_payment_link_creation') loop
...
-- SQL-origin preparation notifications must also respect a hold. Return NULL
-- suppresses only this delivery, never financial processing or event recording.
create function public.suppress_held_order_preparation() returns trigger language plpgsql security definer set search_path=public,pg_temp as $$
begin
